...
def run_testsuite(
    ...
    for i, pid in enumerate(common, 1):
        ...
        logger.info(
            "─── [%d/%d] %s  |  attendu: %-4s  (CDR=%.1g, âge=%s, MMSE=%s)",
            i,
            len(common),
            pid,
            expected,
        ...
        try:
            ...
            predicted_ratio = classify_by_ratio(
                hippo_ratio_pct, patient_gt["age"]
            ).split(" ")[0]
            ...
            logger.info(
                "    volume: %s %-4s  |  ratio: %s %-4s  vol=%.2f cm³/côté  Dice=%.3f  [%.1fs]",
                "✓" if correct_volume else "✗",
                predicted_volume,
                "✓" if correct_ratio else "✗",
                predicted_ratio,
                avg_vol,
                avg_dice,
                elapsed,
            ...
            results.append(
                {
                    ...
                    "hippo_ratio_pct": (
                        round(hippo_ratio_pct, 4)
                        if hippo_ratio_pct is not None
                        else None
                    ),
                    ...
                    "hd95_left": (
                        round(res["hd95_left"], 3)
                        if res["hd95_left"] == res["hd95_left"]
                        else None
                    ),
                    "hd95_right": (
                        round(res["hd95_right"], 3)
                        if res["hd95_right"] == res["hd95_right"]
                        else None
                    ),
            ...
            results.append(
                {
                    ...
                    "vol_right_cm3": None,
                    "vol_avg_cm3": None,
                    "brain_volume_cm3": None,
                    "hippo_ratio_pct": None,
                    "asymmetry_pct": None,
                    "dice_left": None,
                    "dice_right": None,
                    "iou_left": None,
...
def _print_method_section(
    ok: list[dict], groups: list[str], label: str, pred_key: str, correct_key: str
) -> None:
...
def _print_summary(results: list[dict]) -> None:
    ...
    avg_dice = (
        sum((r["dice_left"] + r["dice_right"]) / 2 for r in ok) / len(ok) if ok else 0
    )
    ...
    _print_method_section(
        ok, groups, "Diagnostic par volume brut", "predicted_volume", "correct_volume"
    )
    _print_method_section(
        ok,
        groups,
        "Diagnostic par ratio hippocampe/cerveau",
        "predicted_ratio",
        "correct_ratio",
    )
...
def _save_results(results: list[dict], output: str | None) -> None:
    ...
    path = (
        Path(output) if output else RESULTS_DIR / f"results_{len(results)}patients.csv"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", type=int, default=None, help="Graine aléatoire pour reproductibilité"
    )
    parser.add_argument(
        "--group",
        choices=["CN", "MCI", "AD"],
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="Fichier CSV de sortie"
    )
